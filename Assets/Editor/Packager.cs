...
public class Packager {
    ...
    static void Build(bool resverUpdate)
    {
        ...
        // Lua打包
        EditorUtility.DisplayProgressBar("打包", "Lua打包中...", 0.1f);
        BuildLua();

        // 数据打包
        EditorUtility.DisplayProgressBar("打包", "数据打包中...", 0.2f);
        BuildData();
...
//                fileList += fileName + "|" + md5 + "|" + size + "\n";
    ...
    static void BuildLua()
    {
        ...
        BuildPipeline.BuildAssetBundles(ABPATH, abList.ToArray(), BuildAssetBundleOptions.ChunkBasedCompression, EditorUserBuildSettings.activeBuildTarget);
    ...
    static void BuildData()
    {
        ...
        foreach (FileInfo file in files)
        {
            ...
            File.WriteAllText(newPath, File.ReadAllText(file.FullName));
        ...
        AssetDatabase.Refresh();
        BuildPipeline.BuildAssetBundles(ABPATH, new AssetBundleBuild[] { build }, BuildAssetBundleOptions.ChunkBasedCompression, EditorUserBuildSettings.activeBuildTarget);
